...
// bundle-analyzer 는 배포할 때와 개발할 떄의 분석모드를 다르게 해준다.
...
const config: webpack.Configuration = {
    ...
    rules: [
      {
        ...
        options: {
          presets: [
            //바꿔줄 때의 바벨의 설정을 말한다. 세 가지 프리셋 모두 필요!
            [
              '@babel/preset-env',
              {
          ...
          env: {
            //css에서 emotion 이라는 css style이 있는데, 이를 가능하게 해준다.
            development: {
  ...
  output: {
    //dist 라는 폴더 안에, 위에서 entry:app 에 설정해준 파일들을 만들어 넣어준다.
  ...
  devServer: {
    ...
    proxy: {
      //프론트에서 api로 시작하는 요청은 주소를 3095로 바꾸어서 보내라. (3095로 취급하겠다)
      '/api/': {
...
} // openAnalyzer 을 true 로 두면 분석 창이 뜬다.

if (!isDevelopment && config.plugins) {
  config.plugins.push(new webpack.LoaderOptionsPlugin({ minimize: true })); //얘는 굳이 안 넣어도 되는데 가끔 최적화되는 애가 있음
  config.plugins.push(new BundleAnalyzerPlugin({ analyzerMode: 'static' }));
} // static 모드는 파일명으로 주소가 나온다.
